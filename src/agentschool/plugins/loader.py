...
from agentschool.config.paths import get_config_dir
from agentschool.coordinator.agent_definitions import (
    AGENT_COLORS,
    EFFORT_LEVELS,
    ISOLATION_MODES,
    MEMORY_SCOPES,
...
from agentschool.plugins.schemas import PluginManifest
from agentschool.plugins.types import LoadedPlugin, PluginCommandDefinition
from agentschool.skills.loader import _parse_skill_markdown
...
def get_project_plugins_dir(cwd: str | Path) -> Path:
    ...
    path = Path(cwd).resolve() / ".agentschool" / "plugins"
    path.mkdir(parents=True, exist_ok=True)
    return path
...
def _load_plugin_hooks(path: Path) -> dict[str, list]:
    ...
    if not path.exists():
        return {}
    from agentschool.hooks.schemas import (
        AgentHookDefinition,
...
def _load_plugin_mcp(path: Path) -> dict[str, object]:
    ...
    if not path.exists():
        return {}
    from agentschool.mcp.types import McpJsonConfig
...
def _load_plugin_tools(path: Path, manifest: PluginManifest) -> list:
    ...
    from agentschool.tools.base import BaseTool
