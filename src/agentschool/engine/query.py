...
from agentschool.config.paths import get_data_dir
from agentschool.engine.messages import ConversationMessage, ToolResultBlock
...
from agentschool.hooks import HookEvent, HookExecutor
from agentschool.permissions.checker import PermissionChecker
from agentschool.services.tool_outputs import tool_output_inline_chars, tool_output_preview_chars
...
def _merge_tool_result_metadata(
    ...
    """Merge structured tool result metadata back into session state."""
    if tool_metadata is None or not result_metadata:
        return
    tool_metadata.update(result_metadata)
...
async def run_query(
    ...
    """
    from agentschool.services.compact import (
        AutoCompactState,
...
async def _execute_tool_call(
    ...
    _merge_tool_result_metadata(context.tool_metadata, result.metadata)
