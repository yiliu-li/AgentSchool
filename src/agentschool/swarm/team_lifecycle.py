"""Persistent team lifecycle management for AgentSchool swarms.

Teams are stored as JSON files on disk:
    ~/.agentschool/teams/<name>/team.json
...
from agentschool.swarm.mailbox import get_team_dir
from agentschool.swarm.types import BackendType
...
async def _kill_orphaned_teammate_panes(team_name: str) -> None:
    ...
    """
    from agentschool.swarm.registry import get_backend_registry
    from agentschool.swarm.spawn_utils import is_inside_tmux
    from agentschool.swarm.types import is_pane_backend
...
class TeamLifecycleManager:
    ...
    Persists team metadata to ``~/.agentschool/teams/<name>/team.json``.
    ...
    def list_teams(self) -> list[TeamFile]:
        """Return all teams found in ``~/.agentschool/teams/``, sorted by name."""
        base = Path.home() / ".agentschool" / "teams"
        if not base.exists():
            return []
