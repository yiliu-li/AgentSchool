"""File-based async message queue for leader-worker communication in AgentSchool swarms.

Each message is stored as an individual JSON file:
    ~/.agentschool/teams/<team>/agents/<agent_id>/inbox/<timestamp>_<message_id>.json
...
def get_team_dir(team_name: str) -> Path:
    """Return ~/.agentschool/teams/<team_name>/"""
    base = Path.home() / ".agentschool" / "teams" / team_name
    base.mkdir(parents=True, exist_ok=True)
    return base


def get_agent_mailbox_dir(team_name: str, agent_id: str) -> Path:
    """Return ~/.agentschool/teams/<team_name>/agents/<agent_id>/inbox/"""
