"""Permission sync protocol for leader-worker coordination in AgentSchool swarms.
...
Paths:
    ~/.agentschool/teams/<teamName>/permissions/pending/<id>.json
    ~/.agentschool/teams/<teamName>/permissions/resolved/<id>.json
"""
...
from agentschool.swarm.lockfile import exclusive_file_lock
from agentschool.swarm.mailbox import (
    MailboxMessage,
    TeammateMailbox,
...
def get_permission_dir(team_name: str) -> Path:
    """Return ~/.agentschool/teams/{teamName}/permissions/"""
...
async def get_leader_name(team_name: str | None = None) -> str | None:
    ...
    """
    from agentschool.swarm.team_lifecycle import read_team_file_async
...
async def handle_permission_request(
    ...
    Args:
        ...
        checker: An already-configured :class:`~agentschool.permissions.checker.PermissionChecker`.
