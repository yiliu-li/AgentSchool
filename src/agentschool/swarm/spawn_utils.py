...
TEAMMATE_COMMAND_ENV_VAR = "AGENTSCHOOL_TEAMMATE_COMMAND"
...
_TEAMMATE_ENV_VARS = [
    ...
    "REQUESTS_CA_BUNDLE",
    "CURL_CA_BUNDLE",
    # --- AgentSchool-native provider settings --------------------------------
    ...
    "AGENTSCHOOL_API_FORMAT",
    "AGENTSCHOOL_BASE_URL",
    "AGENTSCHOOL_MODEL",
    "OPENAI_API_KEY",
]
...
def get_teammate_command() -> str:
    ...
    Resolution order:
    1. ``AGENTSCHOOL_TEAMMATE_COMMAND`` environment variable — allows the
       operator to point at a specific binary or wrapper script.
    2. The current Python interpreter running the ``agentschool`` module.
       This keeps spawned teammates on the same venv/source tree as the
       leader process.
    3. The ``agentschool`` entry-point on PATH (installed package fallback).
    """
    ...
    entry_point = shutil.which("agentschool")
    if entry_point:
        return entry_point
    return "python"
...
def build_inherited_cli_flags(
    *,
    ...
    # --- Model override ----------------------------------------------------
    # "inherit" means use the parent's model via the AGENTSCHOOL_MODEL env var.
...
def build_inherited_env_vars() -> dict[str, str]:
    ...
    Always includes ``AGENTSCHOOL_AGENT_TEAMS=1`` plus any provider/proxy
