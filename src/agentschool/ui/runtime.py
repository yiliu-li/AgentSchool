"""Shared runtime assembly for headless and interactive UI hosts."""
...
from agentschool.api.client import AnthropicApiClient, SupportsStreamingMessages
...
from agentschool.commands import CommandContext, CommandResult, MemoryCommandBackend, create_default_command_registry
...
from agentschool.hooks import HookEvent, HookExecutionContext, HookExecutor, load_hook_registry
from agentschool.hooks.hot_reload import HookReloader
from agentschool.mcp.client import McpClientManager
from agentschool.mcp.config import load_mcp_server_configs
from agentschool.learning import run_learning_workflow
...
from agentschool.tools import ToolRegistry, create_default_tool_registry
from agentschool.keybindings import load_keybindings
...
class RuntimeBundle:
    ...
    def current_settings(self):
        ...
        We persist most settings to disk (``~/.agentschool/settings.json``), but
...
def _resolve_api_client_from_settings(settings) -> SupportsStreamingMessages:
    ...
    def _safe_resolve_auth():
        try:
            ...
            print(
                ...
                "  Run `agentschool auth login` to set up authentication, or set the\n"
    ...
    if settings.api_format == "copilot":
        from agentschool.api.copilot_client import COPILOT_DEFAULT_MODEL
...
async def build_runtime(
    *,
    ...
    """Build the shared runtime for an AgentSchool session."""
    ...
    if settings.sandbox.enabled and settings.sandbox.backend == "docker":
        from agentschool.sandbox.session import start_docker_sandbox
...
async def close_runtime(bundle: RuntimeBundle) -> None:
    ...
    from agentschool.sandbox.session import stop_docker_sandbox
...
async def handle_line(
    ...
    *,
    ...
    if parsed is not None:
        ...
        if result.learning_workspace is not None:
            summary = await run_learning_workflow(
                workspace=result.learning_workspace,
                model=bundle.engine.model,
                max_turns=bundle.engine.max_turns,
                status_printer=print_system,
            )
            await print_system(
...
async def _render_command_result(
    ...
    if result.replay_messages and render_event is not None:
        ...
        from agentschool.engine.stream_events import AssistantTextDelta, AssistantTurnComplete
