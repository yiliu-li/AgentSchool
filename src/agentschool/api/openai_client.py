...
from agentschool.api.errors import (
...
def _default_headers_for_base_url(base_url: str | None) -> dict[str, str] | None:
    """Return provider-specific default headers for OpenAI-compatible gateways."""
    normalized = _normalize_openai_base_url(base_url)
    if not normalized or "openrouter.ai" not in normalized.lower():
        return None
    return {
        "HTTP-Referer": os.environ.get("AGENTSCHOOL_OPENROUTER_REFERER", "https://github.com"),
        "X-Title": os.environ.get("AGENTSCHOOL_OPENROUTER_TITLE", "AgentSchool"),
    }
...
class OpenAICompatibleClient:
    ...
    def __init__(self, api_key: str, *, base_url: str | None = None, timeout: float | None = None) -> None:
        ...
        default_headers = _default_headers_for_base_url(normalized_base_url)
        if default_headers:
    ...
    @staticmethod
    def _translate_error(exc: Exception) -> AgentSchoolApiError:
        status = getattr(exc, "status_code", None)
        msg = str(exc)
