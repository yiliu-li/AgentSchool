...
from agentschool.api.errors import AuthenticationFailure, AgentSchoolApiError, RateLimitFailure, RequestFailure
...
def _build_codex_headers(token: str, *, session_id: str | None = None) -> dict[str, str]:
    ...
    headers = {
        ...
        "originator": "agentschool",
        "User-Agent": f"agentschool ({platform.system().lower()} {platform.machine() or 'unknown'})",
...
def _translate_status_error(status_code: int, message: str) -> AgentSchoolApiError:
...
class CodexApiClient:
    ...
    async def _stream_once(self, request: ApiMessageRequest) -> AsyncIterator[ApiStreamEvent]:
        body: dict[str, Any] = {
            ...
            "store": False,
            "stream": True,
            "instructions": request.system_prompt or "You are AgentSchool.",
    ...
    def _translate_error(exc: Exception) -> AgentSchoolApiError:
        if isinstance(exc, AgentSchoolApiError):
            return exc
        if isinstance(exc, httpx.HTTPStatusError):
