"""GitHub Copilot API client for AgentSchool.
...
_VERSION = "0.1.0"  # AgentSchool version for User-Agent
...
class CopilotClient:
    ...
    github_token:
        ...
        token is loaded from ``~/.agentschool/copilot_auth.json``.
    enterprise_url:
        ...
        *,
        ...
        if not token:
            raise AuthenticationFailure(
                "No GitHub Copilot token found. Run 'agentschool auth copilot-login' first."
            )
        ...
        default_headers: dict[str, str] = {
            "User-Agent": f"agentschool/{_VERSION}",
