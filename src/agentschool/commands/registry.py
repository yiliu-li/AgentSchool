...
from agentschool.bridge import get_bridge_manager
...
from agentschool.config.settings import Settings, display_model_setting, load_settings, save_settings
from agentschool.engine.messages import ConversationMessage
from agentschool.engine.query_engine import QueryEngine
from agentschool.learning import prepare_learning_workspace
...
from agentschool.output_styles import load_output_styles
...
from agentschool.services import (
...
from agentschool.services.session_backend import DEFAULT_SESSION_BACKEND, SessionBackend
from agentschool.skills import load_skill_registry
from agentschool.tasks import get_task_manager
from agentschool.plugins.types import PluginCommandDefinition

if TYPE_CHECKING:
    from agentschool.learning import LearningWorkspace
    from agentschool.state import AppStateStore
    from agentschool.tools.base import ToolRegistry
...
class CommandResult:
    ...
    learning_workspace: "LearningWorkspace | None" = None
...
def create_default_command_registry(
    ...
    async def _version_handler(_: str, context: CommandContext) -> CommandResult:
        del context
        try:
            version = importlib.metadata.version("agentschool")
        except importlib.metadata.PackageNotFoundError:
            version = "0.1.7"
        return CommandResult(message=f"AgentSchool {version}")
    ...
    async def _init_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        if not claudemd.exists():
            claudemd.write_text(
                ...
                "- Use AgentSchool tools deliberately.\n"
        ...
        for relative, content in (
            (
                ...
                "# Project AgentSchool Config\n\nThis directory stores project-specific AgentSchool state.\n",
            ),
            (
        ...
        if not created:
            return CommandResult(message="Project already initialized for AgentSchool.")
    ...
    async def _learn_handler(args: str, context: CommandContext) -> CommandResult:
        goal = args.strip()
        if not goal:
            return CommandResult(message="Usage: /learn GOAL")
        workspace = prepare_learning_workspace(
            goal_name=goal,
            root=Path(context.cwd) / ".agentschool" / "learn",
            session_cwd=context.cwd,
        )
        return CommandResult(
            message=f"Starting learning run in {workspace.topic_dir}",
    ...
    async def _login_handler(args: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(message="Stored API key in ~/.agentschool/settings.json")
    ...
    async def _keybindings_handler(_: str, context: CommandContext) -> CommandResult:
        from agentschool.keybindings import get_keybindings_path, load_keybindings
    ...
    async def _privacy_settings_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        lines = [
            ...
            "- storage: local files under ~/.agentschool and project .agentschool",
        ]
        return CommandResult(message="\n".join(lines))
    ...
    async def _upgrade_handler(_: str, context: CommandContext) -> CommandResult:
        ...
        return CommandResult(
            message=(
                ...
                "- uv sync --extra dev\n"
                "- uv pip install -e .\n"
                "- no frontend terminal build is required"
            )
        )
    ...
    registry.register(
        SlashCommand("exit", "Exit AgentSchool", _exit_handler, aliases=("quit",))
    )
    registry.register(SlashCommand("clear", "Clear conversation history", _clear_handler))
    registry.register(SlashCommand("version", "Show the installed AgentSchool version", _version_handler))
    ...
    registry.register(SlashCommand("init", "Initialize project AgentSchool files", _init_handler))
    registry.register(
        SlashCommand(
            "bridge",
    ...
    registry.register(
        SlashCommand(
            "learn",
            "Start a fully agentic learning run for a goal",
            _learn_handler,
            remote_invocable=False,
            remote_admin_opt_in=True,
        )
    ...
    registry.register(SlashCommand("release-notes", "Show recent AgentSchool release notes", _release_notes_handler))
...
def _memory_backend_for_context(context: CommandContext) -> MemoryCommandBackend:
    ...
    return MemoryCommandBackend(
        label="AgentSchool project memory",
