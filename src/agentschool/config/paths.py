"""Path resolution for AgentSchool configuration and data directories.

Follows XDG-like conventions with ~/.agentschool/ as the default base directory.
"""
...
def get_config_dir() -> Path:
    ...
    Resolution order:
    1. AGENTSCHOOL_CONFIG_DIR environment variable
    2. ~/.agentschool/
    """
    env_dir = os.environ.get("AGENTSCHOOL_CONFIG_DIR")
    if env_dir:
...
def get_config_file_path() -> Path:
    """Return the path to the main settings file (~/.agentschool/settings.json)."""
...
def get_data_dir() -> Path:
    ...
    Resolution order:
    1. AGENTSCHOOL_DATA_DIR environment variable
    2. ~/.agentschool/data/
    """
    env_dir = os.environ.get("AGENTSCHOOL_DATA_DIR")
    if env_dir:
...
def get_logs_dir() -> Path:
    ...
    Resolution order:
    1. AGENTSCHOOL_LOGS_DIR environment variable
    2. ~/.agentschool/logs/
    """
    env_dir = os.environ.get("AGENTSCHOOL_LOGS_DIR")
    if env_dir:
...
def get_project_config_dir(cwd: str | Path) -> Path:
    """Return the per-project .agentschool directory."""
    project_dir = Path(cwd).resolve() / ".agentschool"
