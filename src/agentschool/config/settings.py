"""Settings model and loading logic for AgentSchool.

Settings are resolved with the following precedence (highest first):
1. CLI arguments
2. Environment variables (ANTHROPIC_API_KEY, AGENTSCHOOL_MODEL, etc.)
3. Config file (~/.agentschool/settings.json)
4. Defaults
"""
...
from agentschool.hooks.schemas import HookDefinition
from agentschool.mcp.types import McpServerConfig
...
class DockerSandboxSettings(BaseModel):
    ...
    image: str = "agentschool-sandbox:latest"
...
def _profile_from_flat_settings(settings: "Settings") -> tuple[str, ProviderProfile]:
    ...
    profile = ProviderProfile(
        ...
        default_model=settings.model or defaults.get("openrouter", ProviderProfile(
            label="OpenRouter",
            provider="openrouter",
            api_format="openai",
            auth_source="openrouter_api_key",
...
class Settings(BaseModel):
    """Main settings model for AgentSchool."""
    ...
    def merged_profiles(self) -> dict[str, ProviderProfile]:
        ...
        for name, raw_profile in self.profiles.items():
            ...
            if builtin is not None and not profile.allowed_models and builtin.allowed_models:
                profile = profile.model_copy(update={"allowed_models": list(builtin.allowed_models)})
            merged[name] = profile
        return merged
    ...
    def resolve_profile(self, name: str | None = None) -> tuple[str, ProviderProfile]:
        ...
        profile_name = (name or self.active_profile or "").strip() or "openrouter"
    ...
    def resolve_api_key(self) -> str:
        ...
        ``agentschool auth copilot-login`` and this method is not called.
        ...
        raise ValueError(
            "No API key found. Set ANTHROPIC_API_KEY, OPENROUTER_API_KEY, or OPENAI_API_KEY "
            "(for openai-format "
            "providers) environment variable, or configure api_key in "
            "~/.agentschool/settings.json"
        )
    ...
    def resolve_auth(self) -> ResolvedAuth:
        ...
        if auth_source in {"codex_subscription", "claude_subscription"}:
            ...
            if binding is None:
                raise ValueError(
                    f"No external auth binding found for {auth_source}. Run 'agentschool auth "
...
def _apply_env_overrides(settings: Settings) -> Settings:
    ...
    explicitly configure the corresponding field.  ``AGENTSCHOOL_*`` env vars
    ...
    # --- model ---
    agentschool_model = os.environ.get("AGENTSCHOOL_MODEL")
    if agentschool_model:
        updates["model"] = strip_ansi_escape_sequences(agentschool_model)
    ...
    # --- base_url ---
    agentschool_base = os.environ.get("AGENTSCHOOL_BASE_URL")
    if agentschool_base:
        updates["base_url"] = agentschool_base
    ...
    max_tokens = os.environ.get("AGENTSCHOOL_MAX_TOKENS")
    if max_tokens:
        updates["max_tokens"] = int(max_tokens)

    timeout = os.environ.get("AGENTSCHOOL_TIMEOUT")
    if timeout:
        updates["timeout"] = float(timeout)

    max_turns = os.environ.get("AGENTSCHOOL_MAX_TURNS")
    if max_turns:
        updates["max_turns"] = int(max_turns)

    context_window_tokens = os.environ.get("AGENTSCHOOL_CONTEXT_WINDOW_TOKENS")
    if context_window_tokens:
        updates["context_window_tokens"] = int(context_window_tokens)

    auto_compact_threshold_tokens = os.environ.get("AGENTSCHOOL_AUTO_COMPACT_THRESHOLD_TOKENS")
    ...
    api_format = os.environ.get("AGENTSCHOOL_API_FORMAT")
    if api_format:
        updates["api_format"] = api_format

    provider = os.environ.get("AGENTSCHOOL_PROVIDER")
    if provider:
        updates["provider"] = provider

    sandbox_enabled = os.environ.get("AGENTSCHOOL_SANDBOX_ENABLED")
    sandbox_fail = os.environ.get("AGENTSCHOOL_SANDBOX_FAIL_IF_UNAVAILABLE")
    sandbox_backend = os.environ.get("AGENTSCHOOL_SANDBOX_BACKEND")
    sandbox_docker_image = os.environ.get("AGENTSCHOOL_SANDBOX_DOCKER_IMAGE")
...
def load_settings(config_path: Path | None = None) -> Settings:
    ...
    if config_path is None:
        from agentschool.config.paths import get_config_file_path
