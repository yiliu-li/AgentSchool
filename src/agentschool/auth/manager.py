"""Unified authentication manager for AgentSchool providers."""
...
# Providers that AgentSchool knows about.
_KNOWN_PROVIDERS = [
    "anthropic",
    "anthropic_claude",
    "openrouter",
...
class AuthManager:
    ...
    Reads/writes credentials via :mod:`agentschool.auth.storage` and keeps
    ...
    def save_settings(self) -> None:
        ...
        from agentschool.config import save_settings
    ...
    def switch_provider(self, name: str) -> None:
        ...
        if name in _KNOWN_PROVIDERS:
            self.use_profile(_PROFILE_BY_PROVIDER.get(name, "openrouter" if name == "openrouter" else ("openai-compatible" if name == "openai" else "claude-api")))
            return
        raise ValueError(
