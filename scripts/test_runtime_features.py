#!/usr/bin/env python3
"""E2E tests for runtime features: retry, skills, parallel tools, path permissions.
...
def _run_agentschool(*args: str, timeout: int = 90) -> subprocess.CompletedProcess:
...
async def test_api_retry_config() -> tuple[bool, str]:
    ...
    from agentschool.api.client import MAX_RETRIES, RETRYABLE_STATUS_CODES, _get_retry_delay
...
async def test_skills_loaded() -> tuple[bool, str]:
    ...
    from agentschool.skills.bundled import get_bundled_skills
...
async def test_parallel_tools_code() -> tuple[bool, str]:
    ...
    from agentschool.engine.query import run_query
    import inspect
...
async def test_path_permissions_deny() -> tuple[bool, str]:
    ...
    from agentschool.config.settings import PermissionSettings, PathRuleConfig
