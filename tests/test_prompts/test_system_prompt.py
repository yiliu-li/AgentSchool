"""Tests for agentschool.prompts.system_prompt."""

from __future__ import annotations

from agentschool.prompts.environment import EnvironmentInfo
...
def _make_env(**overrides) -> EnvironmentInfo:
    defaults = dict(
        ...
        python_executable="/home/user/.agentschool-venv/bin/python",
        virtual_env="/home/user/.agentschool-venv",
        is_git_repo=True,
        git_branch="main",
        hostname="testhost",
    )
...
def test_build_system_prompt_contains_environment():
    ...
    assert "/home/user/.agentschool-venv/bin/python" in prompt
    assert "Virtual environment: /home/user/.agentschool-venv" in prompt
...
def test_build_system_prompt_custom_prompt():
    ...
    assert "AgentSchool" not in prompt


def test_build_system_prompt_default_includes_base():
    env = _make_env()
    prompt = build_system_prompt(env=env)
    assert "AgentSchool" in prompt
